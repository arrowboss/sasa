...
import getGuarantorTitleOptions from '@salesforce/apex/UWFlowController.getGuarantorTitleOptions';
/* ════════════════════════════════════════════════════════════════════
   FLOW CONFIG
   ════════════════════════════════════════════════════════════════════ */
const FLOW_CONFIG = [
...
export default class UnderwritingFlow extends NavigationMixin(LightningElement) {
...
Promise.all([
    ...
    getGuarantorTypeOptions(),
     getGuarantorTitleOptions()
])
.then(([data, gData, gtOptions, titleOptions]) => {
    ...
    this.borrowerRows  = (gData.borrowerRows  || []).map(r => ({ ...r, _dirty: false }));
    this.guarantorRows = (gData.guarantorRows || []).map(r => ({ ...r,  _dirty: false }));
    this.guarantorTypeOptions = (gtOptions || []).map(o => ({ label: o.label, value: o.value }));
    this.guarantorTitleOptions = (titleOptions || []).map(o => ({ label: o.label, value: o.value }));
...
if (s.isGuarantorTable) {
   ...
   return {
    ...
    guarantorTitleOptions: this.guarantorTitleOptions,
...
async redirectAndClose() {
    try {
        // ✅ Step 1: Redirect to Loan
        this.navigateToLoan();

        // ✅ Step 2: Close current subtab
        const tabInfo = await getFocusedTabInfo();
        await closeTab(tabInfo.tabId);

    } catch (error) {
        console.error('Redirect/Close error:', error);
    }
}
...
onGuarantorChange(e) {
    ...
    let rows = rowType === 'borrower'
        ? [...this.borrowerRows]
        : [...this.guarantorRows];

    rows = rows.map((row, i) => {

        // ✅ SPECIAL CASE: Primary Owner (affects ALL rows)
        if (field === 'Primary_Owner__c' && val === true) {
            return {
                ...row,
                Primary_Owner__c: i === idx,
                _dirty: true   // mark all rows dirty
            };
        }

        // ✅ Only selected row updated
        if (i === idx) {
            return {
                ...row,
                [field]: val,
                _dirty: true   // mark only this row dirty
            };
        }
...
// ✅ Only changed rows
const changedRows = [...this.borrowerRows, ...this.guarantorRows]
    .filter(r => r._dirty);

// ✅ Build payload ONLY for changed rows
const guarantorPayload = changedRows.map(r => ({
    Id:                  r.Id,
    Business_Owner__c:   r.Business_Owner__c,
    Ownership__c:        r.Ownership__c,
    Title__c:            r.Title__c,
    Guarantor_Type__c:   r.Guarantor_Type__c,
    Primary_Owner__c:    r.Primary_Owner__c,
    Credit_Score__c:     r.Credit_Score__c,
    BO_Mgmt_Exp_Yrs__c:  r.BO_Mgmt_Exp_Yrs__c
}));
...
_saveAndNavigate(navigateFn) {
    ...
    this.borrowerRows = this.borrowerRows.map(r => ({
        ...r,
        _dirty: false
    }));

    this.guarantorRows = this.guarantorRows.map(r => ({
        ...r,
        _dirty: false
    }));
